...
- **Registry**: um bando de dados de pacotes disponíveis para o `Node.js`
...
O **GitHub Package Registry** é um serviço de hospedagem de pacotes, semelhante ao `npm`, que permite que você hospede seus pacotes e códigos em um único lugar. Ele suporta pacotes do `Node.js`, `Ruby`, `Java`, `.NET` e imagens `Docker`.
...
8. Clique no botão **Generate token** no final da página.
...
10. Crie ou edite o aquivo `~/.npmrc` e substitua `PERSONAL-ACCESS-TOKEN` pelo seu token. O arquivo `~/.npmrc` é responsável pelas configurações globais do `npm`.
...
1. [`gpr-package`](https://github.com/robertoachar/gpr-package) é o repositório que será publicado no GitHub Package Registry
2. [`gpr-project`](https://github.com/robertoachar/gpr-project) é o projeto que utilizará o pacote publicado
...
- Ler o [artigo sobre a introdução do GitHub Package Registry](https://github.blog/2019-05-10-introducing-github-package-registry/) no blog do GitHub.

- Ler a [documentação do GitHub Package Registry](https://help.github.com/en/articles/about-github-package-registry)

- Visitar a [página do GitHub Package Registry](https://github.com/features/package-registry) para conhecer mais sobre a nova funcionalidade e se inscrever no _open beta_.
