...
- Contruindo o primeiro monorepo
...
**Monorepo** é uma estratégia de desenvolvimento de software onde o código de vários projetos são agrupados em um mesmo repositório.

Essa técnica não é nova, muito pelo contrário, ela já exite há muitos anos, ela apenas ganhou um nome (como tudo hoje em dia 🤭).

### Vantagens

- **Ambiente de desenvolvimento**: facilidade para executar o ambiente de desenvolvimento. Basta clonar um único repositório, instalar as dependências de todos os projetos com um único comando e executar o projeto com outro comando.

- **Reutilização de código**: funcionalidades compartilhadas podem ser facilmente extraídas em um projeto separado e reutilizadas em outros projetos.
...
- **Gerenciamento de versões**: em múltiplos repositórios, quando um projeto é versionado, todos os outros projetos que dependem dele precisam ser atualizados. No monorepo, não há essa necessidade, pois o código está _linkado_.

### Desvantagens

- **CI/CD**: realizar o _Continuous Integration_ ou _Continuous Delivery_ com monorepo pode ser complicado. Não é fácil identificar qual projeto foi alterado para iniciar os processos de **CI/CD** apenas para ele.

- **Segurança**: como todos os projetos estão no mesmo repositório, os desenvolvedores têm acesso a tudo. Isso pode causar alguns problemas de segurança. **Obs.:** há projetos em que esse cenário não é considerado uma desvantagem.

- **Equívocos**: com muitos projetos em um mesmo repositório, se não prestar atenção no diretório que está aberto no Terminal, você acaba executando ou instalando alguma dependência no projeto errado.
...
Workspaces é uma funcionalidade do Yarn que permite agrupar múltiplos projetos em um único repositório. Ele é o grande facilitador para a criação de monorepos. Basta informar quais são os diretórios no arquivo `package.json` para que o Yarn seja capaz de instalar as dependências e _linkar_ os projetos locais.
...
Nesse tutorial, iremos construir um monorepo com dois projetos extremamente simples.
...
Esse comando gera o arquivo `package.json` na raíz do projeto:
...
1. Criar os diretórios `calc` e `sum` na raíz do repositório e adicionar o nome dos dois projetos no campo `workspaces` no arquivo `package.json`. O problema dessa maneira é que sempre que formos adicionar ou excluir projetos do _workspace_, precisamos lembrar de alterar o arquivo `package.json`.
...
Crie um diretório `sum` dentro do diretório `packages`, abra o diretório `sum` no Terminal e digite `yarn init -y`. Esse comando inicializa um novo projeto `Node` no diretório `sum`.
...
Crie um diretório `calc` dentro do diretório `packages`, abra o diretório `calc` no Terminal e digite `yarn init -y`. Esse comando inicializa um novo projeto `Node` no diretório `calc`.
...
O Yarn Workspaces já faz o trabalho de _linkagem_ dos projetos automaticamente para nós. Dê uma olhada no diretório _node_modules_.
...
Altere o arquivo `index.js` do projeto `sum`.
...
Altere o arquivo `index.js` do projeto `calc`.
...
### Configurando o script do projeto `calc`

Edite o arquivo `package.json` do projeto `calc` e adicione o script `start`.
...
Com apenas um comando, todas as dependências de todos os projetos foram instaladas.
